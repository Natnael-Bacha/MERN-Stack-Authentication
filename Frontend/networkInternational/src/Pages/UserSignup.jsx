...
const UserSignup = () => {
  ...
  const validateForm = () => {
    ...
    const requiredFields = ['firstName', 'middleName', 'lastName', 'email', 'license', 'password', 'confirmPassword'];
    requiredFields.forEach(field => {
      if (!formData[field].trim()) {
        newErrors[field] = `${field === 'license' ? 'License number' : field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, ' $1')} is required`;
        isValid = false;
      }
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const allFields = ['firstName', 'middleName', 'lastName', 'email', 'license', 'password', 'confirmPassword'];
    const touchedFields = {};
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/userAuth/userSignup`, formData);
      ...
      if (error.response) {
        ...
        switch (status) {
          case 409:
            setSubmitError('An account with this license number already exists. Please use a different license number or sign in instead.');
            break;
          case 400:
            if (message === "Passwords should match") {
              setSubmitError('The passwords you entered do not match. Please check and try again.');
            } else {
              setSubmitError('There was a problem creating your account. Please check your information and try again.');
            }
            break;
          case 500:
    ...
    <div className="signup-container">
      <div className="signup-card">
        <div className="signup-header">
          ...
          <p className="signup-subtitle">Join NIGT's Automotive Network</p>
        </div>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input 
                type="text" 
                id="middleName"
                className={`form-input ${errors.middleName && touched.middleName ? 'error' : ''}`}
                ...
                disabled={isSubmitting}
              />
              {errors.middleName && touched.middleName && <span className="error-message">{errors.middleName}</span>}
            </div>
          </div>
